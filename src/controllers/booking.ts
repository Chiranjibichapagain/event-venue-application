...
import { VenueDocument } from '../modals/Venue';
...
export const updateBooking = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (booking) {
      ...
      await Booking.findByIdAndUpdate(bookingId, updatedBooking, { new: true }).then(
        (updated: VenueDocument) => {
          res.status(200).json({
